...
static void 
SwapHeader (NoffHeader *noffH) //.COFF es de UNIX		.NOFF es de NachOS
{
...
AddrSpace::AddrSpace (AddrSpace* fatherSpace) {
	...
	ASSERT (1==1);
	printf("Constructor addrspace");
	/*
	pageTable = new TranslationEntry[numPages];
	
	for (unsigned int i = 0; i < numPages; ++i) {
		pageTable[i].virtualPage = i;	  // Como es virtual es lineal (0,1,2,3...)
		if (i < (numPages - tamanioPila)) {  // Para el code & data segment realiza el if, el espacio del la pila se hará en else
